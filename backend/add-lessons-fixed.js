...
db.serialize(function() {
  ...
  // Clear old lessons and insert new ones
  db.run('DELETE FROM lessons', function(err) {
    if (err) console.error('Error clearing lessons:', err);
    
    const insertLesson = db.prepare(`INSERT INTO lessons VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
    ...
    // Show summary
    db.all('SELECT course_id, COUNT(*) as count FROM lessons GROUP BY course_id', [], function(err, rows) {
      console.log('\n📚 Lessons per course:');
      for (var j = 0; j < rows.length; j++) {
        ...
        console.log('   ' + courseNames[row.course_id] + ': ' + row.count + ' lessons');
      }
